...
def _get_start_end(genome_file: str, df_in: pd.DataFrame, flag_verbose: bool, tolerance: int) -> pd.DataFrame:
    df = df_in.copy()
...
def execute(TIRLearner_instance) -> pd.DataFrame:
    df = _get_start_end(TIRLearner_instance.genome_file_path, TIRLearner_instance["base"],
                        TIRLearner_instance.flag_verbose, TE_TOLERANCE)
