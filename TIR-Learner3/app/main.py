...
class TIRLearner:
	def __init__(self, genome_file_path: str, genome_name: str, species: str, TIR_length: int,
		...
		self.processed_de_novo_result_file_name: str = PROCESSED_DE_NOVO_RESULT_FILE_NAME_FORMAT_STR.format(
			genome = self.genome_name, step = '')
			
		self.processed_de_novo_result_file_name_tirvish: str = PROCESSED_DE_NOVO_RESULT_FILE_NAME_FORMAT_STR.format(
			genome = self.genome_name, step = '_tirvish')
			
		self.processed_de_novo_result_file_name_grf: str = PROCESSED_DE_NOVO_RESULT_FILE_NAME_FORMAT_STR.format(
			genome = self.genome_name, step = '_grf')
	...
	def __mount_working_dir(self):
		#Use of tmp is always a bad idea on HPC and isn't helpful on any other system
		...
		self.working_dir_path = os.path.join(self.output_dir_path, SANDBOX_DIR_NAME)
	...
	def execute_m4(self):
		...
		if self.__progress_check(current_progress) and SKIP_TIRVISH not in self.additional_args:
			...
			self["TIRvish"] = os.path.abspath(self.processed_de_novo_result_file_name_tirvish)
			run_TIRvish.execute(self)
			...
			if not CHECKPOINT_OFF in self.additional_args:
				...
				#shutil.copy(f'{self["TIRvish"]}.fxi', f'{checkpt_name}.fxi')
		...
		if self.__progress_check(current_progress) and SKIP_GRF not in self.additional_args:
			...
			if not CHECKPOINT_OFF in self.additional_args:
				...
				#shutil.copy(f'{self["GRF"]}.fxi', f'{checkpt_name}.fxi')
			
			#self.__save_checkpoint_file()

		'''
		This is never actually needed and was helpful only for loading both whole files into memory.
		These two parts should always have been written separately and loaded/processed separately by the CNN
		current_progress = [4, 5]
		if self.__progress_check(current_progress):
			print("Module 4, Step 5: Combine TIRvish and GRF results")
			with open(self.processed_de_novo_result_file_name, 'wb') as wfd:
				for f in [self["TIRvish"], self["GRF"]]:
					if os.path.exists(f): #Handle skips
						with open(f, 'rb') as fd:
							shutil.copyfileobj(fd, wfd)
			
			self["combined_results_file"] = os.path.abspath(self.processed_de_novo_result_file_name)
